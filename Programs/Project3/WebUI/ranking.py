...
def signup():
    ...
    return redirect(url_for('results',query1=query_url))
...
def results(query1):
    ...
    if len(url_list) == 0:
        url_list = list()
        url_list.append("NO RESULTS FOUND")
        url_dict = dict()
    end = time.time()
    
    return render_template('results.html', title=author,url_list = url_list,url_dict=urls_dict,times=(end-start), nos = len(url_list))
...
def process_query(query):
	...
	for word in words:
        ...
        #print "\n\n\nUnionDict",query,"\n\n\n"
        #raw_input()
	return [(x, unionDict[x]) for x in sorted(unionDict, key = unionDict.get, reverse = True)][:10]
...
def combine(result, key_result, base_result):
	...
	for doc in sets:
                ...
                #for strs in query.split(' '):
                #    if strs.lower() in urls:
                #        url_score += 50
                tf_idf = a + b + c + d + e 
                #print "Url ",doc    
                #print "Common ",str(a)    
                ...
                #print "Prox ",str(e)    
                #print "PR ",str(d)    
                #print "Total ",str(tf_idf)

                #raw_input()
                #tf_idf = a + b + c + d + e + url_score
                #if doc == "49/98":
	...
	return [x for x in sorted(tf_idf_res, key = tf_idf_res.get, reverse = True)][:25]
...
def proximity_search(query):
	...
	token = [stemmer.stem(x) for x in token]
        stop_words = set(stopwords.words('english'))
	token = [word.strip().lower() for word in token if word not in stop_words]
...
def getsnippet(fileName, query):
	...
	with open("../Indexer/WEBPAGES_RAW/" + fileName, "r") as f:
		content = f.read()
		try:
