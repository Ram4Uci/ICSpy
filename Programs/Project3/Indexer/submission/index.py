...
def tokenizeHTML(root, doc_ID):
	...
	for header in root.findAll(['h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'a', 'title']):
                header_text = header.get_text().strip().encode('utf-8').decode("ascii","ignore").lower()
                
                if header_text != "": 
                        if header.name in ['h1','title']:   
                                key_list += header_text + " "
                        else:
                                base_list += header_text + " "
                        #print header_text
                        #raw_input("header-text")
                        #print key_list
        #raw_input("Text")
	text = re.sub(r'[\W_]+', ' ', key_list)
	text1 = re.sub(r'[\W_]+', ' ', base_list)
        text = ' '.join([w for w in text.split(" ") if w not in stop_words])
	...
	stems = stem_tokens(tokens, stemmer)
	stems1 = stem_tokens(tokens1, stemmer)

	for key in stems1:
		newdict = {}
		doc_dicts = base_dict.get(key,newdict)
		doc_dicts[doc_ID] = doc_dicts.get(doc_ID, 0) + 1
		base_dict[key] = doc_dicts
...
def calculate_tf_idf():
	...
	for letter in list_of_letters:
		new_index_name = "index_file_" + letter + "_new"
		index_name = "index_file_" + letter
		my_collection = db[index_name]
		new_collection = db[new_index_name]
...
def dump_fancy_index():
	...
	for word in sorted(base_dict): 
		my_record = dict()
		my_record["word"] = word
		my_record["index"] = base_dict[word]
		n_val = N
		num_docs = len(base_dict[word])
		idf = math.log(N, 10) - math.log(num_docs, 10)
		my_record["idf"] = idf

		for doc in base_dict[word]:
			tf = base_dict[word].get(doc, 0) 
			tf_idf = math.log((1 + tf), 10) * idf
			base_dict[word][doc] = tf_idf
			my_record["index"] = base_dict[word]
		# record_id = my_collection.insert(my_record)
		
		record_id = my_collection1.insert(my_record)
